...
import { LayoutDashboard, MessagesSquare, Gamepad2, TrendingUp, Moon, Sun } from "lucide-react";
...
export default function MobileNav() {
  ...
  return (
    <>
      ...
      <nav className="fixed bottom-3 left-3 right-3 z-30 flex items-center justify-around gap-1 rounded-2xl border border-navy-900/8 bg-white/70 px-2 py-2 shadow-glass-lg backdrop-blur-2xl dark:border-white/8 dark:bg-navy-900/70 md:hidden">
        {NAV.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
